...
def run_module():
    ...
    def get_download_links():
        ...
        for patch in patches['data']['solutions']:
            ordered_files = filter_ordered_files(patch, substring='-patch-FP.iso')
    ...
    try:
        ...
        result['message'] = f"Missing required parameter: {ke}"
        result["failed"] = True
        result['changed'] = False
        module.fail_json(**result)
    except Exception as e:
      
        result['message'] = f"Error during patch download: {str(e)}"
        result["failed"] = True
        result['changed'] = False
        module.fail_json(**result)
